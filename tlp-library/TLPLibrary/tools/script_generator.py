...
@LastEditTime: 2020-03-18 14:32:39
@Description:
'''
...
def get_run_parameter():
    ...
    parse.add_argument("-t", "--type", help="脚本类型, 可选参数为'import'或'inference'，默认为'import'。", type=str, metavar="", choices=('import', 'inference'))
    parse.add_argument("-o", "--output", help="生成脚本输出的路径，默认存储在系统的临时文件夹下，当输入路径时，请使用正确的路径分隔符'\\\\'(windows)或'/'(unix)。", type=str, metavar="")
    parse.add_argument("-n", "--name", help="生成脚本的名字，脚本类型默认生成为'import_region_and_label.py'或'inference_region_and_label.py'。", type=str, metavar="")

    args = parse.parse_args()

    type = args.type if args.type else 'import'
    output = args.output if args.output else tempfile.gettempdir()
    name = args.name if args.name else 'import_region_and_label.py' if not args.type or args.type == 'import' else 'inference_region_and_label.py'
...
def main():
    ...
    if type == 'import':
        ...
        copy_template_file(os.path.join(os.path.dirname(__file__), 'inference_script_template'), target)
    else:
        pass
