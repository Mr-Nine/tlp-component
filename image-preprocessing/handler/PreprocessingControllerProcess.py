...
@LastEditTime: 2019-12-11 20:26:41
@Description:
'''
...
class PreprocessingControllerProcess(Process):

    send_thread_over_queue = queue.Queue()

    config = Config()

    def __init__(self, name, ws, pending_image_queue, controller_process_state_queue, result_thread_state_queue, work_result_queue):
        super(PreprocessingControllerProcess, self).__init__()

        self.name = name

        self.__ws = ws
        self.__pending_image_queue = pending_image_queue
        self.__controller_process_state_queue = controller_process_state_queue
        self.__result_thread_state_queue = result_thread_state_queue
        self.__work_result_queue = work_result_queue
    ...
    def run(self):
        ...
        try:
            logging.info("preprocessing controller process start, name:%s" % self.name)
                ...
                if os.path.exists(self.__save_file_path):
                    ...
                    if pending_image_list:
                        for image in pending_image_list:
                            self.__pending_image_queue.put(image)
            ...
            while self.__running.isSet():
                ...
                while True:
                    try:
                        image = self.__pending_image_queue.get_nowait()
                        break
                ...
                if image:
                    ...
                    work = PreprocessingWorkThread(thread_name, self.__ws, image, self.__sub_thread_array, self.__work_result_queue)
                    work.start()
            ...
            pending_image_list = []
            while not self.__pending_image_queue.empty():
                pending_image_list.append(self.__pending_image_queue.get())
            ...
            self.__controller_process_state_queue.put((True, "save pending success."))
